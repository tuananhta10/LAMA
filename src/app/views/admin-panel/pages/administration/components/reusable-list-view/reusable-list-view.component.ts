...
@Component({
  selector: 'admin-reusable-list-view',
  ...
  */
  searchDataSource(): void {
    ...
    .filter(el => {
      ...
      if(this.searchBy.toLowerCase() === 'yes')
        return JSON.stringify(source).toLowerCase().includes('true');

      else if(this.searchBy.toLowerCase() === 'no')
        return JSON.stringify(source).toLowerCase().includes('false');

      else return JSON.stringify(source).toLowerCase().includes(this.searchBy.toLowerCase());
    });
  ...
  exportAsXLSX(type?):void { 
    ...
    const modifyObject = (el: any) => {
      ...
      sorted.forEach((item, _i) => {
        if(index(item) !== -1) {
          result[findTitle(item)] = /*el[item];*/ typeof el[item] !== 'boolean' && typeof el[item] !== 'object' ? el[item]
