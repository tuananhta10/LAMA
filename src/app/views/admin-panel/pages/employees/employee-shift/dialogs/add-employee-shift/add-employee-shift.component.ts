...
export class AddEmployeeShiftComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.serviceScheduleForm = this.formBuilder.group({
      ...
      client_id: [this.data ? this.data?.client_service_schedule[0]?.client_id : ''],
      client_funding_id: [/*this.data  ? this.data?.client_service_schedule[0]?.client_funding_id : ''*/],  
      //service_location: [this.data?.service_location || 'ACT'],
      is_recurring: [this.data?.client_service_schedule[0]?.is_recurring],
      recurring_interval: [this.data?.client_service_schedule[0]?.recurring_interval || 'Daily'],
      recurring_end_date: [this.data?.client_service_schedule[0]?.recurring_end_date || null],
      ...
      start_time: [this.data ? this.data?.client_service_schedule[0]?.start_time : null],
      end_time: [this.data ? this.data?.client_service_schedule[0]?.end_time : null],
      total_hours: [this.data ? this.data?.client_service_schedule[0]?.total_hours : 0],
      admin_hours: [this.data ? this.data?.client_service_schedule[0]?.admin_hours : 0],
      service_hours: [this.data ? this.data?.client_service_schedule[0]?.service_hours : 0],  
      ttp: [this.data ? this.data?.client_service_schedule[0]?.ttp : false],
      support_item: [this.data ? this.data?.client_service_schedule[0]?.support_item: ''],
      shift_rate: [this.data ? this.data?.client_service_schedule[0]?.shift_rate: ''],
      editable_rate_value: [this.data ? this.data?.client_service_schedule[0]?.editable_rate_value : 0],
    });

    this.hourForm = this.formBuilder.group({
      activity: [this.data ? this.data?.client_service_schedule[0]?.activity : ''],
      additional_roster_comment: [this.data ? this.data?.client_service_schedule[0]?.additional_roster_comment : ''],
      a_h_calculator: [this.data ? this.data?.client_service_schedule[0]?.a_h_calculator : ''],
      status: [this.data ? this.data?.client_service_schedule[0]?.status : 'scheduled'],
      billable: [this.data ? this.data?.client_service_schedule[0]?.billable : true],  
      override_availability: [this.data ? this.data?.client_service_schedule[0]?.override_availability : true],  
      override_qualifications: [this.data ? this.data?.client_service_schedule[0]?.override_qualifications : true],  
      override_qualifications_comment: [this.data ? this.data?.client_service_schedule[0]?.override_qualifications_comment : ''],  
    });

    this.shiftLocationForm = this.formBuilder.group({
      start_shift_address: [this.data ? this.data?.client_service_schedule[0]?.start_shift_address : ''],
      end_shift_address: [this.data ? this.data?.client_service_schedule[0]?.end_shift_address : ''],  
      charge_travel_to_client: [this.data ? this.data?.client_service_schedule[0]?.charge_travel_to_client : ''],
      travel_hours: [this.data ? this.data?.client_service_schedule[0]?.travel_hours : 0],
      travel_distance: [this.data ? this.data?.client_service_schedule[0]?.travel_distance : 0],
      travel_distance_rate: [this.data ? this.data?.client_service_schedule[0]?.travel_distance_rate : 0.85],
      total_travel_km: [this.data ? this.data?.client_service_schedule[0]?.total_travel_km : 0],
      transport_distance: [this.data ? this.data?.client_service_schedule[0]?.transport_distance : 0],
      transport_distance_rate: [this.data ? this.data?.client_service_schedule[0]?.transport_distance_rate : 0.85],
      total_transport: [this.data ? this.data?.client_service_schedule[0]?.total_transport : 0],
      travel_time_rate: [this.data ? this.data?.client_service_schedule[0]?.travel_time_rate : 0],
      travel_total: [this.data ? this.data?.client_service_schedule[0]?.travel_total : 0],
    });

    this.clientTotalForm = this.formBuilder.group({
      rate_type: [this.data ? this.data?.client_service_schedule[0]?.rate_type: ''],
      editable_rate_value: [this.data ? this.data?.client_service_schedule[0]?.editable_rate_value: 0],
      total_hours: [this.data ? this.data?.client_service_schedule[0]?.total_hours : 0],
      calculated_cost: [this.data ? this.data?.client_service_schedule[0]?.calculated_cost : 0],
      expenses_total: [this.data ? this.data?.client_service_schedule[0]?.expenses_total : 0],
      service_fee: [this.data ? this.data?.client_service_schedule[0]?.service_fee : 0],
      travel_total: [this.data ? this.data?.client_service_schedule[0]?.travel_total : 0],
      client_total: [this.data ? this.data?.client_service_schedule[0]?.client_total : 0],
    });
