...
export class CertificatesComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.currentFile = this.data?.certificate?.attachment[0];
  }
  ...
  save(): void {
    if(this.certificateForm.valid && !this.data?.certificate){
    ...
    else if(this.certificateForm.valid && this.data?.certificate){

      let data = {
        ...this.certificateForm.value,
        attachment: this.file,
        employee_id: this.data.employee_id,
        id: this.data?.certificate?.id
      }

      this.adminEnumStore.dispatch({
        type: EmployeeCertificateActionTypes.EDIT_EMPLOYEE_CERTIFICATE,
        payload: data
      });
