...
@Component({
  selector: 'app-reusable-table-referrals',
  animations: [mainAnimations],
  templateUrl: './reusable-table-referrals.component.html',
  providers: [
    {
      provide: NG_VALUE_ACCESSOR,
      useExisting: forwardRef(() => ReusableTableReferralsComponent),
      multi: true
    },
    { provide: DateAdapter, useClass: CustomDateAdapter }
  ],
  styleUrls: ['./reusable-table-referrals.component.scss']
})
export class ReusableTableReferralsComponent implements OnInit {
  ...
  public status: string[] = ["All Referrals", "Draft", "Referral Received", "Contact Made", "Intake Scheduled", "Intake Complete", "Onboard Complete"/*, "SW Meet & Greet"*/];
  public selectedStatus: string = 'All Referrals';
  ...
  ngOnInit(): void {
    this.dataSource.data = [...this.listDataSource.filter(el => el/*.status !== 'Draft' && el.status !== 'Onboard Complete'*/)].slice(0, this.maxRows);
    this.pageNumbers = Math.ceil([...this.listDataSource.filter(el => el/*.status !== 'Draft' && el.status !== 'Onboard Complete'*/)].length / this.maxRows);
    this.paginate = Array(this.pageNumbers).fill(0).map((el, i) => i + 1);
  }
  ...
  */
  ...
  filterStatus(status: string): void {
    const listDataSource = [...this.listDataSource]
    .filter(el => {
      let source = { status: el.status }
      
      if(status === 'All Referrals'){
        return JSON.stringify(source).toLowerCase().includes('')
      }

      else return JSON.stringify(source).toLowerCase().includes(status.toLowerCase());
    });
