...
export class ParticipantDetailsComponent implements OnInit {
  ...
  public status: string[] = [/*"Draft", */"Referral Received", "Contact Made", "Intake Scheduled", "Intake Complete", "Onboard Complete", "Declined/Not Proceeding"/*, "SW Meet & Greet"*/];
  ...
  onRepresentativeAddressChange(event:any){
    console.log("ADDRESS",event);
    this.participantDetailsForm.patchValue({
      representative_address_a: !event?.name?.match('undefined') ? event?.name : event?.suburb,
      representative_suburb: event?.suburb,
      representative_post_code: event?.postcode,
      representative_state: this.stateObj.find(el => el.title.toLowerCase() == event?.state.toLowerCase())['state'],
    });
  }
