...
export class AssignEmployeeComponent implements OnInit {
  ...
  public available: any[] = ['All Employee', 'Available Today', 'Available work schedule on the selected shift'];
    ...
    if(this.showFilter){
      ...
      const listDataSource = [...this.employeeEnums].filter(el => {
        for(let item in data){
          if(data[item] === el[item] && item !== 'qualifications') return true;

          else if(item === 'qualifications'){
            let existingQualification = el['qualification'].find(_el => {
              //console.log("INSIDE", _el, _el?.qualification?.qualification, data['qualifications'])
              return _el?.qualification?.qualification == data['qualifications']
            });
