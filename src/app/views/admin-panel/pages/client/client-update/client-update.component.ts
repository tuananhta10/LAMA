...
export class ClientUpdateComponent implements OnInit, OnDestroy {
  ...
  formStep: number = parseInt(sessionStorage.getItem('clientFormStep')) || 1;
  ...
  clientTypeList = ['client-detail', 'demographics', /*'intake',*/ 'onboarding-notes', 'care-workers', 'medications-and-client-notes', 'service-details', 'contact-details', 'related-docs'];
  ...
  subscribeClient(){
    ...
    this.req = client.subscribe((results: any) => {
      ...
      if(!this.currentStatus)
        this.currentStatus = this.clientForm?.clientDetail?.status?.toLowerCase();

      // set client name
      if(results?.name && results?.name?.first_name && !results?.pending){
        this.clientName = `${results?.name?.first_name} ${results?.name?.last_name}`;
      } 
        
      if(results?.success){
        this.snackBar.open("Successfully Updated Participant", "", {
          duration: 4000,
  ...
  ngOnDestroy(): void {
    sessionStorage.removeItem('clientFormStep');
  ...
  updateClientForm(data){
    ...
    console.log(data, this.currentStatus)
