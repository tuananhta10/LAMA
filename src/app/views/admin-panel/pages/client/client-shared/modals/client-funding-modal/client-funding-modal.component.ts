...
import { ClientGoalDetailsComponent } from '../../../client-details/components/goals/goals-form/client-goal-details/client-goal-details.component';
...
@Component({
  selector: 'app-client-funding-modal',
  templateUrl: './client-funding-modal.component.html',
  styleUrls: ['./client-funding-modal.component.scss'],
})
export class ClientFundingModalComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.partnerDetailsForm = this.formBuilder.group({
      ...
      client_id: [this.data ? this.data.client : ''],
      funding_source_obj: [this.data ? this.data.funding_source_id : '', [Validators.required]],
      ...
      main_branch_obj: [this.data ? this.data.main_branch_obj : ''],
      start_date: [this.data ? this.data.start_date : '', [Validators.required]],
      end_date: [this.data ? this.data.end_date : '', [Validators.required]],
      ...
      used_to_date: [this.data ? this.data.used_to_date : ''],
  ...
  subscribeEnums(){
    ...
    this.req = this.enum$.subscribe((results: any) => {
      ...
      if(results.branch.branchList){
        // results.fundingSource.fundingSourceList.forEach(element => {
        //   element.name = element.full_name
        // });
        this.branches = results.branch.branchList
      }
  ...
  save() {
    if(this.partnerDetailsForm.valid && !this.data){
      ...
      this.partnerDetailsForm.controls['client_id'].setValue(this.data?.client_id);
      this.dialogRef.close(this.partnerDetailsForm.value);
    }
