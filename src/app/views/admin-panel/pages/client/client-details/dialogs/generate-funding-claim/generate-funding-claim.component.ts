...
@Component({
  selector: 'app-generate-funding-claim',
  templateUrl: './generate-funding-claim.component.html',
  styleUrls: ['./generate-funding-claim.component.scss']
})
export class GenerateFundingClaimComponent implements OnInit, OnDestroy {
  ...
  constructor(
    public dialogRef: MatDialogRef<GenerateFundingClaimComponent>,
      ...
      if(timesheetApproval?.generated_claim?.data?.length > 0){
        ...
        setTimeout(() => {
          if(dataInvoice){
            ...
            this.snackBar.open("Successfully generated the funding claim. Please refresh the list if the new claim items was not loaded.", "", {
              duration: 4000,
