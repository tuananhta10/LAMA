...
export class SideProfileComponent implements OnInit {
  ...
  assignGoals(){
    ...
    this.router.navigate([`/admin/clients/details/${this.id}/service-schedule`]);
  ...
  assignCareworker(){
    ...
    sessionStorage.setItem('assignTo', 'true');
    this.router.navigate([`/admin/clients/edit/${this.id}`]);
  }
