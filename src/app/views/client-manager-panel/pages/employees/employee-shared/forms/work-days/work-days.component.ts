...
export class WorkDaysComponent implements OnInit, OnDestroy {
  ...
  isChecked = false;
  isLinear = false;
  weekdays: WorkDaysModel[] = weekdays;
  selectedDates: any[] = [];
  ...
  submit(){
    let employeeCalendarSchedule: any = {
      add: [],
      update: [],
      delete: this.deletedDates
    };

    let updateWeekSchedule: any = {
      update: [...this.weekdays]
    };
    ...
    let data:any = {
      "employee-week-schedule": this.workDaysData["employee_week_schedule"] ? updateWeekSchedule : this.weekdays,
