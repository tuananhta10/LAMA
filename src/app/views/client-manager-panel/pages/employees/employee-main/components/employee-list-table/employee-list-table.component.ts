...
export class EmployeeListTableComponent implements OnInit {
  ...
  ngOnInit(): void {
    //this.employeeList = [];
    //this.filterByStatus(); // filter employee list by status
      ...
      if(results){
        ...
        if(results.employees.employeeUpload){
          if(results.employees.employeeUpload.result.toLowerCase() === 'failed'){
            ...
            this.snackBar.open('Successfully uploaded employee list', "", {
              duration: 4000,
  ...
  deleteEmployeeData(result){
    ...
    /*// update list
    this.clientList = [...this.clientList].filter((el) => !this.selectedRows.find(_el => _el.id === el.id));
  ...
  */
  filterByStatus(item: any){
    const employeeList = [...this.employeeList].filter(el => item.status ? el.status === item?.status : el)
  ...
  */
  ...
  exportAsXLSX(type?):void { 
    ...
    this.excelService.exportAsExcelFile(data, 'employeeList');
