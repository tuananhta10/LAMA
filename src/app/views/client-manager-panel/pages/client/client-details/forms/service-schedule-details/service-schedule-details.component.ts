...
export class ServiceScheduleDetailsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if(this.data){
      this.getClientFunding(this.data?.client_id)
    }
  ...
  subscribeEnums(){
    ...
    this.clientFundingReq = this.clientFundingData$.subscribe((clientFunding: any) => {
      if (clientFunding.clientFundingList.length > 0) {
        setTimeout(() => {
          this.serviceScheduleForm.controls['client_funding_id'].setValue(this.data?.client_funding_id);  
        }, 300);
  ...
  */
  recalculateCost(){
    let support_item = this.serviceScheduleForm.controls['price_list_id'].value;
    let selectedPriceList = [...this.priceListEnum].find(el => el.id == support_item); 
    let rateType = selectedPriceList?.service_type_unit;
